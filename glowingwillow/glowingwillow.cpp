...
#define BRANCH_PIN_3 25  // Branch pointing to led drum 2 (Branch 3)
...
void trunk_rainbow_wipe_shifted(int count) {
  for(int i=0; i<TRUNK_PIXEL_COUNT/2; i++) {
    for (int j=0; j<4; j++) {
      set_trunk_led(j, (count + i + (j*12) )%(TRUNK_PIXEL_COUNT/2), CHSV(i*5, SATURATION, VALUE));
    }
  }
  FastLED.show();
  //FastLED.delay(DELAY);
...
void tree_rgb_sines(float wavelength,float phase_shift_speed_r,float phase_shift_speed_g,float phase_shift_speed_b,float freq_amp_mod) {
  ...
  float amplitude = 1.0; //0...1 -> max brightness
  float phase_offset = 0.0;//wavelength*3.0/4.0; //offset of sine at start [px]
  ...
  for(uint8_t curr_pixel=0; curr_pixel<TRUNK_PIXEL_COUNT/2; curr_pixel++) {
    for (uint8_t j=0; j<4; j++) {//TODO: hardcoded value (number of "logical" strips in trunk)
      ...
      set_trunk_led(j,
                    ...
                    CRGB(int(255.0*
                             amplitude*
                             amplitude_modulation(freq_amp_mod)*
                             wave_propagation(curr_pixel,
                                              phase_offset,
                                              phase_shift_speed_r,
                                              wavelength)),
                         int(255.0*
...
void bounce(){
  for (size_t k = 0; k < 50; k++) {
    for(uint8_t i=0; i<TRUNK_STRIP_COUNT; i++) {
     for (uint8_t j=0; j<TRUNK_PIXEL_COUNT/2; j++) {
       if(j==k){
         trunk_leds[i][j] = CRGB(255,0,0);
         trunk_leds[i][100-j] = CRGB(255,0,0);
       }
       else{
         trunk_leds[i][j] = CRGB(0,255,0);
         trunk_leds[i][100-j] = CRGB(0,255,0);
       }
     }
    }
    FastLED.show();
